...
> With physical access to a standard Android installation which is secured by a 4 digit PIN it is possible to get access to the device in max. 17 hours. This means all the data on Android phones which rely on a 4 digit PIN as security measure are at stake. This poses as a serious threat for businesses especially with bring-your-own-device policies.

> Android offers security with PIN as the best form of security for their devices. As for Androids policy with it is possible to enter a wrong PIN code up to 5 times - then a user would have to wait 30 seconds until he can enter the PIN 5 times again. Android does not offer any security measures for limiting these attempts or increasing the time between the attempt blocks. This makes it possible to attack the PIN code with brute-force. A 4 digit PIN could therefore be cracked with an automated attack in about 17 hours: 35s (=5x entering a PIN: about 1s each then waiting 30s) x 1000 / 5 (= no. of combinations from 0000 to 9999 divided in blocks of 5 each).

> This talk will show a method of how to crack the PIN code fully automated and generic for every Android phone. This will be realized with cheap and available hardware (arduino and webcam, as well as some cables) and a windows program which has been written by the author in c#. The talk describes a method that can easily be reproduced by everybody. No programming or hacking skills will be required for this attack anymore. The costs for the hardware are about 100€.

> With that attack everybody is able to crack any Android phone with a four digit PIN in a reasonable time with reasonable costs. This results in a major security risk for businesses as well as every Android user. If the phone is lost or stolen, a 4 digit PIN simply does not protect the data good enough!

> From the current situation there are only two technical solutions for this security problem:
> - Google changes their security policy and does something like Apple does with their iOS (exponentially increasing time periods between attempts)
> - Implement a maximum number of tries. After all of these are done you need need some other form of authentication to unlock the pin code again (eg.: email to gmail account)

> As for users their only chance is to use stronger pin codes. A 5 digit pin code will increase the time needed to crack the pin to over 110 hours. A 6 digit pin will take over 80 days to brute force (which is really unlikely to be done out in the wild, mostly because you would have to recharge the phone every 5 to 10 hours for at least 3 hours)..

> The best outcome of making this flaw public would be that Google will secure their OS better and implement some measures against this kind of attack.

> The above described lecture will be derived from my master thesis. The software and hardware used in the above described attack is currently in the making with a prototype already ready for demo purposes. The software as well as the hardware will be ready for the talk in November. Screenshots, pictures as well as videos with the prototype can be delivered within the next weeks if needed.
